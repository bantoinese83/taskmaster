...
export function ChartTooltip2({ active, payload, label, formatter, labelFormatter, className }: ChartTooltipProps) {
  if (!active || !payload?.length) {
    return null
  }
      ...
      <div className="grid grid-cols-2 gap-2">
        {payload.map((item, index) => (
          <div key={index} className="flex items-center justify-between gap-2">
            ...
            <div className="text-sm font-medium">{formatter ? formatter(item.value, item.name) : item.value}</div>
          </div>
        ))}
      </div>
    </div>
